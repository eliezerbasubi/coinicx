...
const ChartTimeInterval = () => {
  const currentInterval = useChartSettingsStore((s) => s.interval);
  const bookmarkIntervals = useChartSettingsStore((s) => s.bookmarkIntervals);
  const setSettings = useChartSettingsStore((s) => s.setSettings);

  const onIntervalChange = (interval: CandleSnapshotParameters["interval"]) => {
    setSettings({ interval });
  };
...
const MoreIntervals = () => {
  ...
  const bookmarkIntervals = useChartSettingsStore((s) => s.bookmarkIntervals);
  const setSettings = useChartSettingsStore((s) => s.setSettings);
  const resetBookmark = useChartSettingsStore((s) => s.resetBookmarkIntervals);
